...
//        let oAuthParameters = oAuthDefaultParametersString(consumerKey: cc.key, userKey: uc?.key)
//
//        /// [RFC-5849 Section 3.4.1.3.1](https://tools.ietf.org/html/rfc5849#section-3.4.1.3.1)
//        let signString: String =  (url.query != nil ? url.query! + "&" : "") + oAuthParameters
//
//
        let oAuthParameters = oAuthDefaultParameters(consumerKey: cc.key, userKey: uc?.key)
        ...
        let signingKey: String = rfc3986encode(cc.secret)+"&"
//
        /// [RFC-5849 Section 3.4.2](https://tools.ietf.org/html/rfc5849#section-3.4.2)
        let base = "GET&https%3A%2F%2Fprime.thefirstclinic.com%2FMobileApi%2Fapi%2Fload_online_appointment&oauth_version%3D1.0%26oauth_nonce%3D010011012%26oauth_timestamp%3D012011010%26oauth_signature_method%3DHMAC-SHA1%26oauth_consumer_key%3Dkhaber_1"
        
        let binarySignature = HMAC.calculate(withHash: .sha1, key: signingKey, message: signatureBase).base64EncodedString()
        
        /// [RFC-5849 Section 3.5.1](https://tools.ietf.org/html/rfc5849#section-3.5.1)
        return signString+"&oauth_signature="+binarySignature
    }
    ...
    private static func oAuthDefaultParametersString(consumerKey: String, userKey: String?) -> String
    {
        /// [RFC-5849 Section 3.1](https://tools.ietf.org/html/rfc5849#section-3.1)
        let defaults = "oauth_version=1.0&oauth_nonce=010011012&oauth_timestamp=012011010&oauth_signature_method=HMAC-SHA1&oauth_consumer_key=\(consumerKey)"
        return defaults
    }
}
